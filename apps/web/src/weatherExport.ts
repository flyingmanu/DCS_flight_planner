...
/**
 * Draws the altitude/cloud/wind visualization (sky gradient, gridlines,
 * cloud band, ground line, per-layer wind glyphs) into an arbitrary
 * rectangle of an existing canvas context. Shared by the full PNG export
 * and the dialog's live preview so the two never drift apart.
 */
export function drawWeatherChart(ctx: CanvasRenderingContext2D, weather: MissionWeather, x: number, y: number, width: number, height: number): void {
  ...
  const yOf = (ft: number) => bottom - (Math.min(ft, CHART_TOP_FT) / CHART_TOP_FT) * height;
  ...
  for (let ft = 10000; ft <= CHART_TOP_FT; ft += 10000) {
    ...
    ctx.stroke();
    ctx.fillStyle = "#3a4552";
    ctx.fillText(`${(ft / 1000).toFixed(0)},000 ft`, x + 6, gy - 4);
  }
  ...
  if (oktas > 0 && cloudThickness > 0) {
    ...
    ctx.fillRect(x + 10, cTop, width - 20, cBottom - cTop);
    ctx.strokeRect(x + 10, cTop, width - 20, cBottom - cTop);
  }
  ...
  wind.forEach((layer) => {
    windArrowGlyph(ctx, x + 70, yOf(layer.altitudeFt), layer);
  });
}
...
export function drawWeatherSummary(weather: MissionWeather, missionName: string): HTMLCanvasElement {
  ...
  drawWeatherChart(ctx, weather, chartLeft, chartTop, chartWidth, chartHeight);
...
/**
 * Redraws the live chart preview onto a `<canvas>` already in the DOM,
 * scaling its backing resolution to the element's current CSS size (times
 * devicePixelRatio) so text stays crisp regardless of dialog layout.
 */
export function renderWeatherChartPreview(canvas: HTMLCanvasElement, weather: MissionWeather): void {
  const ctx = canvas.getContext("2d");
  if (!ctx) return;
  ...
  drawWeatherChart(ctx, weather, 0, 0, cssWidth, cssHeight);
}
